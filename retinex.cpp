...
{
    Mat A, B;
    Mat frame, frame2, frame_msr, frame_msrcr;
    frame =  imread("fix.jpg");
      //  resize(frame, frame, Size(frame.size().width * 0.1, frame.size().height * 0.1));
       cv::sort(frame, frame2, SORT_EVERY_ROW+ SORT_DESCENDING);
        imshow("frame_origin", frame2);
        waitKey(0);
