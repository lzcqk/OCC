...
{
	Mat imageLED1;
	imageLED1 = imread("stack_Moment.jpg");
	//resize(imageLED1,imageLED1,Size(1280,960),(0,0),(0,0),INTER_NEAREST);
	//imshow("【原图】", imageLED1);
	...
	Mat imageLED=imageLED1(Rect(X_min, Y_min, X_max - X_min, Y_max - Y_min));
...
//	Mat imageLED3=imageLED1(Rect(X3_min, Y3_min, X3_max - X3_min, Y3_max - Y3_min));
	...
	Mat msgDateoringal=imageLED.col(imageLED.size().height / 2);                                       //msgDateoringal表示中间列像素矩阵
	cout<<"-----------------------------------------"<<endl;
    cout<<"这个LED_ROI的维度是:"<<msgDateoringal.t().size()<<endl;                                                            //一个LED_ROI的维度是 46行1列

	cout<<"-----------------------------------------"<<endl;
    cout <<"原本的中间列像素msgDate = "<< msgDateoringal.t() <<endl;                                   //将消息输出出来
	...
	{
        meanOfPxielRow = mean(imageLED.row(RowOfimgLED), maskOfimgLED.row(RowOfimgLED)).val[0];  //.val[0]表示第一个通道的均值
        RowOfimgLED ++;
       // cout << "值 = "<< meanOfPxielRow <<std::endl;
        *it = meanOfPxielRow;                        //真实的列矩阵是均值
    }
	cout<<"-----------------------------------------"<<endl;
	...
	//cout<<"------------阈值长度"<<local_maxmin_threshold.size()<<endl;

	sample_again: std::cout << "******sample_again   "<<sample_point<<"   次"<<std::endl;
	//cout<<msgDate_resize.rows;
